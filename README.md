...
* Database updates can take into account not only changes in file size but also file metadata change time (`ctime`) or content modification time (`mtime`). Any change to file metadata then triggers a SHA512 checksum recalculation and an update of the file record in the database. For example, if a file’s `ctime` changes but its size and `mtime` remain unchanged, its checksum will not be recalculated when only `--update` is specified. Recalculation requires `--watch-timestamps`. This option is disabled by default because ctime can change frequently, for example, through `chmod` or `chown`, while the file contents remain unchanged
