...
| Флаг | Значение |
|---|---|
| `GOOD` | локальный ответ функции-проверки: да, условие выполнено |
| `NOTGOOD` | локальный ответ функции-проверки: нет, условие не выполнено |
| `BOOLEAN` | маска бинарных флагов: `GOOD | NOTGOOD` |

Пример: функция проверяет, доступен ли путь для чтения. Если проверка выполнена штатно и путь доступен, она может вернуть `SUCCESS | GOOD`. Если проверка выполнена штатно, но путь недоступен, она может вернуть `SUCCESS | NOTGOOD`.

Простое правило: `GOOD` и `NOTGOOD` хорошо подходят функциям, которые по смыслу отвечают на вопрос. Например: `is_*`, `has_*`, `can_*`, `check_*`, `validate_*`. Вызывающий код обычно разбирает такой ответ сразу после вызова. Если текущая функция уже решила, что делать дальше, бинарный ответ считается обработанным. Возвращайте `GOOD` или `NOTGOOD` выше только тогда, когда текущая функция сама тоже обещает ответить «да» или «нет».
...
Из `global_return_status` в результат функции попадают только биты `GLOBAL`. Бинарные ответы `GOOD` и `NOTGOOD` не протекают из глобального статуса между функциями.
...
* если установлен `NOTGOOD`, удаляется `GOOD`;
...
То же правило действует для бинарного слоя: если где-то получилось `GOOD | NOTGOOD`, остаётся `NOTGOOD`. Если что-то вернуло плохой логический результат, итоговый бинарный ответ уже не может считаться хорошим.
...
## Функция-проверка с GOOD и NOTGOOD
...
Сначала проверяйте технический слой. Только после этого разбирайте бинарный ответ. В примере ниже `path_is_readable()` возвращает `GOOD` или `NOTGOOD`, потому что её смысл — ответить на вопрос «доступен ли файл для чтения?». Функция `print_file_if_readable()` уже не отвечает на такой вопрос. Она только решает, печатать файл или нет, поэтому не возвращает наследованные `GOOD` или `NOTGOOD` дальше.
...
Для короткой проверки технического успеха можно использовать `TRIUMPH`. Логика остаётся такой же: технический сбой превращается в `FAILURE` текущей функции, а `GOOD` используется только для локального решения.
...
Главное правило: не воспринимайте `NOTGOOD` как технический сбой. Это отрицательный логический ответ. Технический сбой проверяется через `CRITICAL`, `FAILURE`, `WARNING` или отсутствие ожидаемого `TRIUMPH`. Не протаскивайте `GOOD` и `NOTGOOD` по цепочке автоматически. Если функция получила бинарный ответ и на его основании уже выбрала действие, для этой функции бинарный ответ обработан.
...
Важно: из `global_return_status` в обычный возврат попадают только флаги из `GLOBAL`: `INFO`, `WARNING`, `HALTED`. Локальные бинарные ответы `GOOD` и `NOTGOOD` через глобальный статус не распространяются.
...
4. `GOOD` и `NOTGOOD` используются только для локального логического ответа функции-проверки.
5. `GOOD` и `NOTGOOD` возвращаются наружу только тогда, когда сама текущая функция по смыслу является проверкой и должна ответить «да» или «нет». Это локальные флаги, которые не нужно автоматически наследовать по цепочке из одной функции в другую.
