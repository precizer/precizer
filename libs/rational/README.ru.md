...
6. [Отчеты и логирование](#отчеты-и-логирование)
   * [Цвет и оформление сообщений](#цвет-и-оформление-сообщений)
   * [Сохранение сообщений через REMEMBER](#сохранение-сообщений-через-remember)
...
* `REMEMBER` — передать готовую строку без ANSI-оформления в необязательный обработчик `rational_remember()`
...
### Цвет и оформление сообщений

В строках для `slog()` можно выделять текст маркерами `@{...}`. Они работают вместе с обычными подстановками `printf`, например `%s` и `%d`:
...
При разрешённом оформлении в первой строке полужирным выделяется только `--update`, а во второй всё сообщение становится красным и полужирным. При отключённом оформлении известные маркеры удаляются, и остаётся обычный текст без добавленных ANSI-последовательностей, включая сброс оформления

`UNDECOR` убирает только префиксы логгера, например время и место вызова. На цвет и полужирное начертание этот флаг не влияет. Сам `slog()` пишет в `stdout`, в том числе при уровне `ERROR`. Обычный `printf()`, а также `report()` и `serp()` не обрабатывают эти маркеры

#### Когда включается оформление

Оформлением управляет глобальная атомарная переменная `rational_color_mode`, независимо от режима видимости сообщений `rational_logger_mode`:

| Значение | Поведение |
|---|---|
| `COLOR_MODE_AUTO` | Режим по умолчанию. Разрешает оформление только для терминала или псевдотерминала, если его не отключает окружение |
| `COLOR_MODE_ALWAYS` | Разрешает оформление даже при выводе в файл или pipe, независимо от `NO_COLOR` и `TERM` |
| `COLOR_MODE_NEVER` | Отключает оформление независимо от потока и окружения |

В режиме `AUTO` наличие `NO_COLOR`, даже с пустым значением, или `TERM=dumb` отключает оформление. В остальных случаях используется проверка `isatty()` для конкретного потока. Поэтому обычный вывод в файл, pipe или CI без терминала остаётся без оформления
...
Функция `rational_color_is_enabled(stream)` позволяет проверить решение отдельно для `stdout` и `stderr`. В режиме `AUTO` проверяется текущее подключение потока, если оформление не отключено наличием `NO_COLOR` или значением `TERM=dumb`. Функция сохраняет `errno` и не меняет буферизацию. Возвращаемое значение разрешает оформление, но не доказывает поддержку терминалом конкретных ANSI-последовательностей, 256 цветов или true color. Проверки возможностей через `terminfo` в библиотеке нет

В precizer этот режим задаётся опцией `--color`; описание для пользователя находится в [README программы](../../README.ru.md)

#### Доступные маркеры

Имена чувствительны к регистру и записываются строчными буквами:

| Маркеры | Действие |
|---|---|
| `@{bold}` | Включить полужирное начертание |
| `@{black}`, `@{gray}`, `@{red}`, `@{green}`, `@{yellow}`, `@{blue}`, `@{magenta}`, `@{cyan}`, `@{white}` | Выбрать цвет текста |
| `@{boldblack}`, `@{boldred}`, `@{boldgreen}`, `@{boldyellow}`, `@{boldblue}`, `@{boldmagenta}`, `@{boldcyan}`, `@{boldwhite}` | Включить полужирное начертание и выбрать цвет |
| `@{reset}` | Сбросить цвет и начертание |
| `@{shorttab}` | Вставить последовательность `SHORTTAB` (`\033[?5W`), результат которой зависит от поддержки терминалом; это не обычный символ `\t` |

Для сочетания стилей маркеры записываются подряд: `@{bold}@{red}`. Для этой комбинации также есть `@{boldred}`. Запись `@{bold red}` не распознаётся

После оформленного текста требуется явный `@{reset}`. Логгер только заменяет маркеры: состояние оформления не отслеживается, сброс автоматически не добавляется. Без `@{reset}` цвет и начертание могут сохраниться для последующего вывода, в том числе после перевода строки или следующего вызова `slog()`

#### Подставляемый текст и буквальные маркеры

Маркеры обрабатываются после подстановок `printf` и санитаризации — преобразования небезопасных управляющих байтов в видимый текст. Поэтому маркеры внутри `%s` также применяют оформление:

```c
slog(REGULAR|UNDECOR,"%s\n","@{green}Ready@{reset}");
```

Этот вызов выводит `Ready` зелёным, если оформление разрешено, и обычным текстом в остальных случаях. Выполняется буквальная замена каждого известного маркера на ANSI-последовательность или пустую строку. Неизвестные маркеры, например `@{unknown}`, и незаконченные конструкции, например `@{red`, остаются видимыми. Если внутри такого текста встречается полный известный маркер, он всё равно заменяется: например, при отключённом оформлении `@{unknown @{red}}` превращается в `@{unknown }`

Экранирования маркеров нет. В записи `@@{red}` первый `@` остаётся текстом, а следующий `@{red}` заменяется на включение красного цвета или удаляется. Это относится и к именам файлов или другим данным, переданным через `%s`: буквальное совпадение с известным маркером обрабатывается как оформление

Настоящие байты ESC в формате или аргументах не используются для оформления: санитаризация превращает их в видимое `\x1B` ещё до обработки маркеров, независимо от цветового режима. Для оформления через `slog()` применяется `@{red}`, а не вставка макроса `RED` в строку. Макросы `RED`, `BOLD`, `RESET` и другие из `rational_decoration.h` остаются строками ANSI-последовательностей; сами по себе они не учитывают `rational_color_mode`

### Сохранение сообщений через REMEMBER

Флаг `REMEMBER` позволяет одновременно вывести оформленное сообщение и передать его текст приложению без ANSI-последовательностей оформления. Для этого приложение определяет обработчик:

```c
void rational_remember(const char *message, const int message_length);
```

Например, при разрешённом выводе обычных сообщений:
...
В терминале сообщение будет красным, если оформление разрешено. Обработчик получит `Cannot read archive.db\n`: без маркеров оформления и ANSI, но с завершающим переводом строки. Маркеры из подставляемых аргументов обрабатываются по тем же правилам. Если не указан `UNDECOR`, обычные префиксы логгера также входят в передаваемую строку

Если приложение не определило `rational_remember()`, передача пропускается, а вывод продолжает работать. Флаг не обходит фильтрацию сообщений режимом логгера. Указатель `message` действителен только во время вызова обработчика: для хранения скопируйте `message_length` байтов. Не вызывайте `slog()` из обработчика, чтобы не получить рекурсивное логирование
...
| `DECISION` | маска бинарных флагов: `YES | NO` |
