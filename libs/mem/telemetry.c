...
{
	__atomic_fetch_add(&telemetry.actual_reallocations_counter, 1, __ATOMIC_SEQ_CST);
}
...
{
	printf("Telemetry: Current memory (should be 0): %s\n",bkbmbgbtbpbeb(telemetry.current_bytes));
	...
	printf("Telemetry: Total optimized trick heap memory reallocations number: %s\n",form(telemetry.realloc_optimized_counter));
	printf("Telemetry: Total actual memory reallocations number: %s\n",form(telemetry.actual_reallocations_counter));
	printf("Telemetry: Total memory bytes allocated during execution: %s\n",bkbmbgbtbpbeb(telemetry.total_allocated_bytes));
