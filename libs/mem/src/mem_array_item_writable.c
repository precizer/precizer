...
/**
 * @brief Return a writable pointer to one typed descriptor element
 *
 ...
 * to @ref mem_data_writable. It returns `NULL` for invalid descriptors,
 ...
 * @return Writable element pointer on success. `NULL` when the element cannot be addressed safely
 */
	...
	size_t expected_single_element_size)
{
	...
	void *data = mem_data_writable(memory_structure,expected_single_element_size);
