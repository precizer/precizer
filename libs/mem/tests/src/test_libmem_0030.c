...
Return test_libmem_0030(void)
{
	...
	ASSERT(SUCCESS == m_concat_string(string_buffer,sizeof(empty_suffix),empty_suffix));
	...
	ASSERT(0 == strcmp(m_text(string_buffer),"base"));
	ASSERT(SUCCESS == mem_core_string(SOURCE_FIXED_STRING | TRANSFER_APPEND,string_buffer,0,literal_suffix));
	ASSERT(0 == strcmp(m_text(string_buffer),"base"));
	ASSERT(SUCCESS == mem_core_string(SOURCE_BOUNDED_STRING | TRANSFER_APPEND,string_buffer,sizeof(bounded_suffix),bounded_suffix));
	ASSERT(SUCCESS == m_concat_string(string_buffer,unbounded_suffix));
	ASSERT(SUCCESS == mem_core_string(SOURCE_FIXED_STRING | TRANSFER_APPEND,string_buffer,sizeof(literal_suffix),literal_suffix));
	ASSERT(string_buffer->is_string == true);
