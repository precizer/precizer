...
* Готовая Linux сборка представляет собой исполнимый, статически слинкованный бинарный файл в формате ELF не привязанный к какому-либо определённому дистрибутиву. Файл может быть запущен сразу, практически на любом дистрибутиве Linux и не требует использования каких-либо внешних динамически подгружаемых библиотек. Файл собран CI/CD автосборкой ресурса GitHub, затем сжат с помощью [UPX (архиватора исполняемых файлов)](https://upx.github.io). После этого самораспаковывающийся сжатый бинарный файл вкладывается в ZIP-архив для удобного скачивания. Для использования достаточно распаковать файл из ZIP-архива и запустить.
...
* Автор **НЕ** готов самостоятельно готовить и поддерживать в будущем пакетирование программы **precizer** под _все_ существующие дистрибутивы операционных систем.
...
Результатом сборки будет статически слинкованный бинарный исполняемый файл в формате ELF без каких-либо динамических зависимостей. Этот файл представляет собой всю программу целиком и может быть запущен практически на любом современном дистрибутиве Linux.

В программу интегрированы почти все используемые библиотеки, и по умолчанию программа собирается как статический исполняемый файл. Это сделано для увеличения переносимости и уменьшения зависимостей. Благодаря вышеописанному, программу можно легко собрать на большинстве современных платформ, выполнив несколько команд:
...
Эта сборка создаст единый исполняемый файл, не нуждающийся в отдельных библиотеках. Файл будет оптимизирован под максимальную производительность на текущей аппаратной платформе и будет использовать все возможности локального CPU по-максимому (оптимизация -O3)

Если есть необходимость создать портируемую версию единого исполняемого файла, которая может быть исполненан на большинстве соверменных Linux без тонкой оптимизации под определённый CPU и максимально переносимость между разными поколениями и производителями аппаратных платформ, то достаточно выполнить команду

```sh
make portable
```

Созданный в результате единый исполняемый файл может быть скопирован на любую платформу ч86_
...
### Сборка portable (переносимой версии программы)
...
Если есть причины не устанавливать дополнительные пакеты для самостоятельной сборки приложения, то можно воспользоваться подготовленным решением на базе Docker.
...
Если утилита для автоматизации сборки make отсутствует, то для сборки приложения внутри контейнера достаточно выполнить следующие действия вручную:
...
В результате в текущей директории появится единственный исполняемый, статически слинкованный бинарный файл.

Если с созданным исполняемым файлом есть проблемы, то можно попробовать повысить его переносимость между разными системами и версиями ядер:
...
git clone https://github.com/precizer/precizer.git
cd precizer
docker build --build-arg OS=ubuntu:24.04 --build-arg BUILD=portable -t precizer .
docker create --name precizer precizer
docker cp precizer:/precizer/precizer precizer
docker rm -f precizer
```
