...
`--update` не означает полный пересчёт всех контрольных сумм. Для обычных файлов с уже сохранённой полной SHA512 пересчёт запускается при изменении размера или `mtime`. С `--watch-timestamps` также учитываются изменения `ctime` и количества занятых блоков на диске. Правила защиты архивных файлов через `--lock-checksum` описаны в [примере 9](#пример-9), а их принудительная проверка — в [примере 10](#пример-10)
