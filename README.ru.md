...
Нужно обратить особое внимание, что этот пример относится исключительно к тем файлам, о которых есть запись в БД но к которым по каким-то причинам действительно нет доступа на диске. Это может быть связано с неправильной установкой прав chmod\chown или некорректно смонтированным томом. ВНИМАНИЕ! Если файл, или даже путь к файлу действительно УДАЛЁН, а не просто временно недоступен, то при обновлении БД с параметром `--update` упоминание о таком файле будет однозначно вытерто из базы данных без необходимости указания каких-либо дополнительных параметров!
