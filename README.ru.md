...
- [Непрерывная интеграция и автоматизация](#continuous-integration-and-automation)
- [Кратко о программе](#кратко-о-программе)
- [Скачивание](#скачиваниеdownload)
- [История изменений](#история-изменений)
- [Технические подробности алгоритмов работы](#технические-подробности-алгоритмов-работы)
- [Вопросы и багрепорты](#вопросы-и-багрепорты)
- [Участие в проекте](#участие-в-проекте)
- [Сборка и установка](#сборка-и-установка)
	- [Пакетирование для дистрибутивов](#пакетирование-для-дистрибутивов)
	- [Сборка с помощью Docker](#сборка-с-помощью-docker)
...
- [Основные параметры командной строки](#основные-параметры-командной-строки)
- [Устранение неполадок](#устранение-неполадок-траблшутинг)
...
Для быстрого вызова программы исполняемый файл `precizer` размещается в одном из каталогов, указанных в `$PATH`. Сборки `portable`, `production` и `dynamic-production` создают этот файл в корне проекта; результат `distribution` находится в `.builds/distribution/precizer`

#### macOS

На macOS статическая линковка не поддерживается. Для сборки доступны динамические варианты `distribution` и `dynamic-production`; сжатие UPX не применяется
...
Для сборки без оптимизации под процессор конкретного компьютера используется цель `distribution`:

```sh
make distribution
```

Результат находится в `.builds/distribution/precizer`. Сборка учитывает `CPPFLAGS`, `CFLAGS` и `LDFLAGS` и не удаляет отладочные символы
...
В этом случае готовый файл `precizer` размещается в корне проекта. Для быстрого вызова программы исполняемый файл размещается в одном из каталогов, указанных в `$PATH`
...
Для сборки Windows x64 требуется [MSYS2](https://www.msys2.org/). Команды выполняются в терминале **MSYS2 MSYS**, а не UCRT64 или MINGW64
...
Если обновление требует закрытия терминала, после подтверждения терминал **MSYS2 MSYS** запускается повторно, а команда выполняется ещё раз для завершения обновления. Подробности — в [инструкции MSYS2](https://www.msys2.org/docs/updating/)
...
Получение исходного кода описано в начале раздела [«Самостоятельная сборка»](#самостоятельная-сборка). Все следующие команды выполняются из корня проекта в том же терминале

##### EXE с DLL рядом

Следующие команды собирают программу и размещают рядом с ней необходимую библиотеку MSYS2:
...
Результат — `precizer.exe` и `msys-2.0.dll` в каталоге `package/msys/`. Оба файла должны находиться в одном каталоге. Для запуска на другом компьютере установка MSYS2 не требуется. SQLite, PCRE2 и argp уже включены в EXE

Создание ZIP-архива при необходимости:
...
Предварительно требуются сборка и копирование файлов из предыдущего подраздела. Следующие команды устанавливают дополнительные инструменты и создают EXE со встроенными программой и DLL:
...
Размер файла может быть уменьшен с помощью UPX:
...
- `--watch-timestamps`, `-T` — Учитывать изменения времени модификации `mtime` и изменения метаданных `ctime` в дополнение к размеру файла при определении необходимости пересчитать контрольную сумму
...
`--update` не означает полный пересчёт всех контрольных сумм. Для уже известных обычных файлов пересчёт запускается при изменении размера, а с `--watch-timestamps` — также при изменении временных меток. Правила защиты архивных файлов через `--lock-checksum` описаны в [примере 9](#пример-9), а их принудительная проверка — в [примере 10](#пример-10)
...
Без `--compare-filter` выводятся все три категории. Подробный пример приведён в разделе [«Фильтрация категорий отчёта сравнения»](#фильтрация-категорий-отчёта-сравнения)
