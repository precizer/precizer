...
3. Cкопировать файлы с базами данных host1.db и host2.db на одну из машин и запустить программу с соответствующими параметрами для сравнения:
...
* Алгоритмы программы **precizer** разработаны так, что очень просто поддерживать актуальность содержащихся данных в созданной базе с путями к файлам и их контрольными суммами без пересчёта всего с самого начала. Достаточно запустить программу с параметром _--update_ чтобы в БД попали новые файлы, была удалена информация о стёртых с диска файлах, а для тех файлов, которые подверглись модификациям и их размеры изменились будет пересчитана контрольная сумма SHA512 и обновлена в БД.
* Можно указать опцию, при которой при обновлении БД будет учитываться не только размет изменившихся файлов, но так же и время создания или изменения файлов. Это значит, что изменения любой метаинформации о файле приведёт к пересчёту контрольной суммы SHA512 с последующим обновлением данных о файле в БД.
...
* Программа никогда не меняет, не удаляет, не перемещает и не копирует ни файлы, ни исследуемые директории. Всё что она делает: составляет списки файлов, контрольные суммы содержимого этих файлов и актуализирует их в базе данных. Все изменения происходят исключительно в границах базы данных, представляющей из себя обычный бинарный файл.
...
Полностью готовое к работе решение можно скачать по этой ссылке.

#### Технические подробности сборки
Исполняемый файл представляет из себя статически слинкованый бинарник без каких либо динамических зависимостей. Этот файл представляет из себя всю программу целиком и может быть запущен практически на любом современном дистрибутиве Linux. Собран автосборкой на ресурса github.
