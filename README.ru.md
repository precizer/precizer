...
Допустим, есть две машины у которых в `/mnt1` и в `/mnt2` соответственно, примонтированы диски большого объёма с идентичным содержимым. Стоит задача побайтно проверить, действительно ли содержимое абсолютно идентично или есть различия.
...
В результате работы программы будут рекурсивно исследованы все директории, начиная с `/mnt1`, и в текущей директории будет создана база данных host1.db. Параметр `--progress` визуализирует прогресс и покажет объём пространства и количество исследуемых файлов.
...
* Если любопытно узнать, что содержится внутри базы данных сформированной **precizer**, то можно воспрользоваться удобным GUI [DB Browser for SQLite](https://sqlitebrowser.org).
...
* Готовая Linux сборка представляет собой исполняемый, статически слинкованный бинарный файл в формате ELF не привязанный к какому-либо определённому дистрибутиву. Файл может быть запущен сразу, практически на любом дистрибутиве Linux и не требует использования каких-либо внешних динамически подгружаемых библиотек.

* Файл собран CI/CD автосборкой ресурса GitHub, затем сжат с помощью [UPX (архиватора исполняемых файлов)](https://upx.github.io). После этого самораспаковывающийся сжатый бинарный файл вкладывается в ZIP-архив для удобного скачивания. Для использования достаточно распаковать файл из ZIP-архива и запустить.
...
Сборка программы уже предусмотрена с использованием docker. При этом подготовленно несколько тюнингованных платформ, которые могут быть выбраны в качестве дистрибутива для сборки программы. Среди успешно работающих дистрибутивов:
...
Ознакомиться с подробностями настроек и устанавливаемыми бибилиотеками можно в соответствующих Docker файлах в поддиректории проекта `.docker/`

Для сборки достаточно указать команду `make`, ключевое слово `docker`, дистрибутив `debian` (например) и потом цель `dynamic-production` или другую.

Пример сборки в контейнере:

```sh
make make docker-gentoo-production
```

команда соберт программу в режиме production используя docker контейнер с дистрибутивом Gentoo

```sh
make make docker-ubuntu-production
```

команда соберёт программу в том же режиме `production`, но уже используя дистрибутив Ubuntu

В результате запуска в директории проекта появится исполняемый файл **precizer**, который был собран внутри контейнера. При этом явным преимуществом использования docker является то, что нет необходимости в систему ставить много инструментов для сборки и тестирования, библиотек и их зависимостей. Достаточно запустить докер и получить результат — исполняемый файл. Дальше, нужно определиться с типом этого исполняемого файла. В случае сомнений лучше остановить выбор на `make portable` как на наиболее универсальном варианте.
...
Результатом сборки будет статически слинкованный, самораспаковывающийся сжатый UPX файл в формате ELF, без каких-либо динамических зависимостей. Этот файл представляет собой всю программу целиком и может быть запущен практически на любом современном дистрибутиве Linux.

Файл оптимизирован под **максимальную переносимость**.

Особенности компиляции и линковки: `-static -O2 -mtune=generic`

Альтернатива с использованием докера:
```sh
make make docker-ubuntu-portable
```
...
#### Исполняемый файл с динамически подгружаемыми бибилиотеками и оптимизацией под локальный CPU

```sh
make dynamic-production
```

Результатом сборки будет исполняемый файл размером около **50 Килобайт** в формате ELF. Он будет заточенн под локальный CPU, динамически слинкован с установленными в систему библиотеками, самораспаковывающийся, сжатый UPX. Этот файл может быть успешно собран и запущен на локальном компьютере если в систему были предварительно установлены такие библиотеки как sqlite3, pcre2, argp и fts
...
Альтернатива с использованием докера:
```sh
make make docker-ubuntu-dynamic-production
```
или замените ubuntu на любой из вышеперечисленных дистрибутивов
...
#### Зависимости для сборки на определнных OS
...
Несколько регулярных выражений для игнорирования можно использовать с помощью опций `--ignore` указав их одновременно.
...
DB будет очищена от упоминаний файлов, соответствующих регулярным выражениям из аргументов `--ignore`: "^.\*/path2/.\*" и "diff2/.\*" но пути, соответствующие шаблонам из `--include` останутся в базе данных.
...
В последующих запусках сохраняйте те же шаблоны блокировки при актуализации базы:

```sh
precizer --update --lock-checksum="^archive/2024/.*" /mnt/storage
```

Файлы вне шаблонов обновляются в обычном порядке. Для заблокированных через `--lock-checksum` записей любое расхождение сразу становится заметным.

### Пример 10
Глубокая проверка заблокированных данных с помощью `--rehash-locked`

Опция `--rehash-locked` используется только вместе с `--lock-checksum`. При её включении каждый файл, попавший под шаблон блокировки и уже присутствующий в базе данных, перечитывается, для него заново вычисляется контрольная сумма SHA512 и выполняется сравнение с сохранённым значением. Это позволяет выполнить дополнительный аудит неизменяемых архивов ценой увеличенного дискового ввода-вывода. Наличие или отсутствие параметра `--watch-timestamps` не влияет на работу `--rehash-locked`. Если пересчитанная сумма и размер совпадают с записью в БД, файл считается консистентным; если при этом на диске отличаются ctime/mtime, новые значения сохраняются в базе данных.
