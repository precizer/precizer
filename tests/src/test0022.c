	...
	const char *expected)
{
	...
	run(m_copy_string(temp_buffer,input));

	if(SUCCESS & status)
	{
		run(remove_trailing_slash(temp_buffer));
		...
		if((SUCCESS & status) && temp_buffer->string_length != expected_length - 1U)
		{
			status = FAILURE;
		}

		if((SUCCESS & status) && temp_buffer->length != expected_length)
		{
			status = FAILURE;
		}
	}
...
Return test0022(void)
{
	...
	ASSERT(SUCCESS == m_resize(non_string_path,sizeof("data////")));

	char *non_string_path_data = m_data(char,non_string_path);
	ASSERT(non_string_path_data != NULL);
	IF(SUCCESS == status)
	{
		memcpy(non_string_path_data,"data////",sizeof("data////"));
	}
	...
	non_string_path_data = m_data(char,non_string_path);
	ASSERT(non_string_path_data != NULL);
	IF(SUCCESS == status)
	{
